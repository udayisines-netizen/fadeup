...
const OUT = process.env.QA_OUT ?? '/opt/fadeup/docs/frontend/artifacts/r5r-final'
...
const ROUTES = [
  ...
  { name: 'shop', path: '/_preview/r5r/s/side-agency' },
  { name: 'barber', path: '/_preview/r5r/s/side-agency/b/e9cf978d-f12e-4b82-872d-a2d1430136fb' },
  { name: 'booking', path: '/_preview/r5r/s/side-agency/book' },
  { name: 'book', path: '/_preview/r5r/book' },
  { name: 'queue', path: '/_preview/r5r/queue' },
  { name: 'appointments', path: '/_preview/r5r/appointments' },
  { name: 'profile', path: '/_preview/r5r/profile' },
  { name: 'pro-dashboard', path: '/_preview/r5r/pro', authed: true },
  { name: 'pro-calendar', path: '/_preview/r5r/pro/calendar', authed: true },
  { name: 'pro-customers', path: '/_preview/r5r/pro/customers', authed: true },
  { name: 'pro-analytics', path: '/_preview/r5r/pro/analytics', authed: true },
  { name: 'pro-retention', path: '/_preview/r5r/pro/retention', authed: true },
  { name: 'pro-profile', path: '/_preview/r5r/pro/profile', authed: true },
]
...
for (const vp of VIEWPORTS) {
  for (const route of ROUTES) {
    ...
    */
    ...
    // Pro routes sit behind RequireAuth: sign in as the local QA owner, then
    // return to the route under test so the probe measures the real page.
    if (route.authed) {
      await page.waitForTimeout(1500)
      if (page.url().includes('/login')) {
        // Never a committed credential: the sweep signs in only with what the
        // operator provides for THIS run, against a deletable local QA login.
        ...
        await page.waitForURL('**/_preview/r5r/pro/**', { timeout: 20000 }).catch(() => {})
        await page.goto(`${BASE}${route.path}`, { waitUntil: 'domcontentloaded', timeout: 30000 })
      }
    }
