/**
 * PLAT-2 — comparaison des deux empreintes de `/platform`.
 *
 * L'écart ADMIS et UNIQUE de ce lot dans la barre de navigation, ce sont les
 * cinq liens neufs : ils changent `navLinks`, `textLength` et `bodyTextHead`
 * sur toutes les routes gardées, et c'est exactement ce que le lot devait
 * faire. Le script les neutralise puis exige l'identité sur TOUT le reste.
 *
 * Usage : node e2e/plat2/compare-baseline.mjs <avant.json> <apres.json>
 */
import { readFileSync } from 'node:fs'

const NEW_LINKS = ['/platform/support', '/platform/moderation', '/platform/sales', '/platform/field', '/platform/posters']
const NAV_TEXT = 'Support Moderation Sales Field Posters '
const FIELDS = ['title', 'htmlTheme', 'bodyTheme', 'bodyFont', 'headings', 'tabLinks', 'tables', 'buttons', 'inputs', 'hasSupportBanner']

const before = JSON.parse(readFileSync(process.argv[2] ?? 'docs/reports/plat2/avant/avant.json', 'utf8'))
const after = JSON.parse(readFileSync(process.argv[3] ?? 'docs/reports/plat2/apres/apres.json', 'utf8'))
...
for (const [x, y] of before.map((r, i) => [r, after[i]])) {
  ...
  if (JSON.stringify(navWithoutNew) !== JSON.stringify(x.dom.navLinks)) {
    d['dom.navLinks (hors 5 liens neufs)'] = [x.dom.navLinks, navWithoutNew]
  }
...
console.log(`empreintes identiques (les 5 liens neufs neutralisés) : ${identical}`)
