/**
 * PLAT-3 — axe sur les QUATRE écrans du lot, à 1440 px et à 390 px.
 *
 * Le relevé porte aussi le DÉBORDEMENT HORIZONTAL et le nombre de cibles
 * tactiles sous 44 px : la densité tactile de la console est un chantier
 * consigné par PLAT-2 §13.6, et une exigence qu'on ne mesure pas n'est pas tenue.
 *
 * Usage : QA_BASE=http://127.0.0.1:4680 node e2e/plat3/platform-plat3-axe.mjs <outdir>
 */
import { chromium } from '@playwright/test'
import AxeBuilder from '@axe-core/playwright'
import { mkdirSync, writeFileSync } from 'node:fs'

const BASE = process.env.QA_BASE ?? 'http://127.0.0.1:4680'
const OUT = process.argv[2] ?? '/tmp/plat3-axe'
mkdirSync(OUT, { recursive: true })

const SCREENS = [
  ['settings', '/platform/settings', true],
  ['promotions', '/platform/promotions', true],
  ['funnel', '/platform/funnel', true],
  ['worker', '/platform/worker', true],
  // Deux écrans HÉRITÉS, relevés pour la comparaison avant/après : ce lot ne
  // doit pas leur ajouter une violation.
  ['login', '/platform/login', false],
  ['overview', '/platform', true],
]
