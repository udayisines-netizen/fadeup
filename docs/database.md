...
- Public (anon) access path for booking-page org lookup by `slug` — later, public-
  booking lot.
- `staff_profiles`, `services`, `appointments`, `invitations` — later lots, per
  `CLAUDE.md`'s LOT sequencing. Nothing in this lot builds toward them beyond the
  `organizations`/`memberships`/`locations` foundation they'll attach to.
- An "an org always has ≥1 owner" invariant (see `memberships` above).
- Any writer for `audit_logs` — no feature in this lot produces audit events yet; the
  table and its RLS exist so the first feature that needs to record one has a safe place
  to write to (via its own `SECURITY DEFINER` function or `service_role`, never a
  blanket client `INSERT` policy).
