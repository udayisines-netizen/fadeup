...
## Current phase
Phase 4 — Customer app (home, appointments, favorites, rebook). Starting now. Will replace `CustomerHomePage`'s minimal body with the real priority stack (queue > appointment > rebook > discover) and build `get_my_appointments`/`get_my_favorites`-style curated RPCs per PLAN.md section 4, decision 2 (narrow RPCs, not broad RLS on shop-internal tables).
