...
#   scripts/disposable-db-test.sh --seed FILE           # + load pre-upgrade rows BEFORE MASTER, so a backfill has real data to migrate
...
# --seed runs BETWEEN the base replay and MASTER. That window is the only
# place a lot's backfill can be tested honestly: it lets the database hold
# realistic PRE-upgrade rows, so the backfill has something to migrate instead
# of running against an empty schema and reporting success vacuously.
if [[ -n "$SEED_FILE" ]]; then
  echo "==> seeding pre-upgrade data: $SEED_FILE"
  if ! docker exec -i -e PGOPTIONS='--client-min-messages=warning' "$CONTAINER" \
