-- FadeUp — PLAT-1 (4/4) : la chaîne de publication suit la même question que
-- son entrée.
--
-- LE DÉFAUT, trouvé en relisant le lot et prouvé en transaction annulée.
-- `publish_external_professional` a reçu la garde `marketplace.publish`, qui
-- ouvre la publication au COMMERCIAL — c'est ce que le fondateur a tranché.
-- Mais elle appelle deux fonctions qui portaient leur propre garde, restée à
-- l'ancien niveau :
--
--   create_external_professional            → is_platform_admin()
--   refresh_prospect_publication_eligibility → has_platform_role([owner, admin])
--
-- Dans une fonction SECURITY DEFINER, `auth.uid()` reste celui de l'APPELANT :
-- ces gardes internes se réévaluent donc pour le commercial, et le refusent.
...
-- commercial ou d'admin » était FAUX en pratique, alors que la garde d'entrée
-- disait oui. Une garde d'entrée qui ment est pire qu'une garde absente.
--
-- CE QUI CHANGE. Les trois fonctions posent `marketplace.publish` à la place
-- de leur ancien niveau. `sweep_prospect_publication_eligibility` est alignée
-- avec sa jumelle : laisser les deux divergentes serait un piège pour PLAT-2.
--
-- CE QUI NE CHANGE PAS. Le bras « worker » (`auth.uid() is null and
-- session_user = 'prospect_worker'`) est repris VERBATIM, commentaires
-- compris : c'est lui qui laisse passer la connexion propre du Worker V2 sans
-- ouvrir la porte à un jeton PostgREST.
--
-- À APPLIQUER EN postgres (propriétaires vérifiés : les trois fonctions lui
-- appartiennent). Après 20260911100100.
