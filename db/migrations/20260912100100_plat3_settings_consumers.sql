-- FadeUp — PLAT-3 (2/4) : les consommateurs des défauts plateforme.
--
-- À APPLIQUER EN postgres. `public.book_public_appointment` appartient à
-- `postgres` (vérifié), et c'est un `create or replace` à signature inchangée :
-- l'ACL est conservée. Un `drop` + `create` obligerait à re-matérialiser les
-- concessions — c'est le piège de P1PRO, rappelé par PLAT-1 §7.
--
-- LE CORPS EST REPRIS VERBATIM DE LA PRODUCTION, à trois endroits près :
--
--   1. la constante `c_max_future_bookings := 5` devient une lecture de
--      `booking.max_future_per_customer` ;
--   2. une garde neuve refuse un horaire au-delà de `booking.window_days` —
--      elle n'existait NULLE PART côté serveur, et un appel direct réservait
--      sans horizon ;
--   3. les commentaires qui décrivaient ces deux points.
--
-- Le nouveau motif de refus est `fadeup_booking_refusal=beyond_booking_window`,
-- dans le vocabulaire fermé que F4 a posé.
...
AS $function$
declare
  -- MASTER_SPEC §6 : réservations futures simultanées, 5 par défaut, et
  -- fenêtre de réservation à 90 jours. PLAT-3 a posé la table de réglages que
  -- ce commentaire attendait : les deux valeurs viennent désormais de
  -- `platform_settings`, et le `coalesce` garde le comportement d'avant si la
  -- clé disparaît. Un réglage manquant ne casse pas une réservation.
  v_max_future_bookings integer;
...
begin
  ...
  -- PLAT-3 — LA FENÊTRE DE RÉSERVATION, ENFIN CÔTÉ SERVEUR.
  -- Avant ce lot, `BOOKING_WINDOW_DAYS = 90` ne vivait que dans slots.ts :
  -- le sélecteur de dates s'arrêtait à 90 jours, et un appel direct à cette
  -- RPC réservait à cinq ans. La garde est ici, elle est STRICTEMENT PLUS
  -- STRICTE qu'avant, et elle ne refuse rien que le client proposait déjà.
  v_booking_window_days := coalesce(private.platform_setting_int('booking.window_days'), 90);
  if p_starts_at > now() + make_interval(days => v_booking_window_days) then
    raise exception 'starts_at is beyond the booking window of % days', v_booking_window_days
      using detail = 'fadeup_booking_refusal=beyond_booking_window';
  end if;

  v_max_future_bookings := coalesce(private.platform_setting_int('booking.max_future_per_customer'), 5);
...
-- ---------------------------------------------------------------------------
-- Les heures calmes et les deux délais de relance de prospection
-- ---------------------------------------------------------------------------
--
-- `private.enqueue_prospect_outreach` appartient à `postgres` (vérifié).
-- Corps repris VERBATIM, à quatre littéraux près : 8 et 21 (heures calmes),
-- 8 heures (deuxième touche) et 2 heures (dernière touche avant expiration).
-- La borne d'une heure qui évite une relance juste avant l'expiration reste
-- telle quelle : ce n'est pas un réglage, c'est une cohérence interne.
...
declare
  ...
  v_count integer := 0;
  -- PLAT-3 : les heures calmes et les deux délais de relance viennent de
  -- `platform_settings`. Le `coalesce` rend exactement les littéraux d'avant
  -- ce lot si la clé disparaît. Le NOMBRE DE TOUCHES, lui, reste à trois et
  -- n'est pas réglable : c'est une loi produit (MASTER_SPEC §5).
  v_quiet_start integer := coalesce(private.platform_setting_int('notifications.quiet_hours_start'), 8);
  v_quiet_end integer := coalesce(private.platform_setting_int('notifications.quiet_hours_end'), 21);
  v_touch2_delay integer := coalesce(private.platform_setting_int('notifications.prospect_touch2_delay_hours'), 8);
  v_touch3_lead integer := coalesce(private.platform_setting_int('notifications.prospect_touch3_lead_hours'), 2);
begin
  for v_row in
    select
      r.id as request_id,
  ...
  loop
    ...
    if v_local_hour < v_quiet_start or v_local_hour >= v_quiet_end then
      continue;
    end if;
    ...
    elsif v_row.touches_sent = 1 and now() >= v_row.created_at + make_interval(hours => v_touch2_delay)
    ...
    elsif v_row.touches_sent = 2 and now() >= v_row.expires_at - make_interval(hours => v_touch3_lead) then
