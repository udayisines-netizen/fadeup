-- ============================================================================
-- F3 — is_managed sur la recherche marketplace
-- ============================================================================
--
-- À APPLIQUER EN postgres (propriétaire vérifié de la fonction : postgres —
-- DB_OWNERSHIP règle 2, contrôlé en production le 2026-09-07).
--
-- F3 construit /search. Le contrat de handoff exige qu'un résultat non
-- revendiqué porte son ClaimBadge neutre (« Pas encore géré sur FadeUp ») —
-- c'est le cas le plus fréquent au lancement (F3 §3). Or les 30 colonnes de
-- search_public_professionals (B1) ne portent AUCUN état de gestion :
-- ni l'organisation (aucune colonne de claim), ni la ligne de résultat. Le
-- front ne peut ni l'inventer ni le deviner ligne à ligne (aucune lecture
-- groupée d'admission n'existe — le N+1 de F2 §11 documente déjà ce manque).
--
-- CHANGEMENT, purement additif au contrat : une 31e colonne `is_managed`.
--   · ligne shop   : exists(memberships de l'organisation) — un établissement
--     publié par la chaîne d'acquisition n'a aucun membre ; un établissement
--     géré en a au moins un (mesuré en production : 8 orgs démo à 0 membre,
--     side-agency à 1).
--   · ligne barber : claim_state = 'claimed' — la frontière B1 exacte de
--     get_public_barber.professional_id (jamais un rattachement non confirmé).
--
-- Un type de retour ne s'étend pas par CREATE OR REPLACE (« cannot change
-- return type ») : DROP puis CREATE — d'où la reprise du corps B1 à
-- l'identique (seules les trois expressions is_managed sont ajoutées) et la
-- repose EXPLICITE de l'ACL et du COMMENT. Aucune table, aucune policy,
-- aucune autre fonction touchée. search_public_organizations n'est PAS
-- modifiée : V2_DATA_CONTRACT §7 la donne redondante, F3 ne la consomme pas.
-- ============================================================================

begin;

drop function public.search_public_professionals(text, text, text, text, double precision, double precision, double precision, integer, integer, boolean, text, integer, integer, text);

create function public.search_public_professionals(p_country text DEFAULT NULL::text, p_city text DEFAULT NULL::text, p_query text DEFAULT NULL::text, p_service_query text DEFAULT NULL::text, p_latitude double precision DEFAULT NULL::double precision, p_longitude double precision DEFAULT NULL::double precision, p_radius_km double precision DEFAULT NULL::double precision, p_min_price_cents integer DEFAULT NULL::integer, p_max_price_cents integer DEFAULT NULL::integer, p_open_now_only boolean DEFAULT false, p_entity_type text DEFAULT NULL::text, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_sort text DEFAULT 'recommended'::text)
...
-- Depuis le durcissement X3 (20260907173000), une fonction neuve de public ne
-- naît exécutable par personne : les grants sont EXPLICITES (DB_OWNERSHIP
-- règle 4) — recalqués sur l'ACL que la fonction B1 portait
-- ({postgres,anon,authenticated,service_role}, concédant postgres).
...
grant execute on function public.search_public_professionals(text, text, text, text, double precision, double precision, double precision, integer, integer, boolean, text, integer, integer, text) to anon;
grant execute on function public.search_public_professionals(text, text, text, text, double precision, double precision, double precision, integer, integer, boolean, text, integer, integer, text) to authenticated;
grant execute on function public.search_public_professionals(text, text, text, text, double precision, double precision, double precision, integer, integer, boolean, text, integer, integer, text) to service_role;

comment on function public.search_public_professionals(text, text, text, text, double precision, double precision, double precision, integer, integer, boolean, text, integer, integer, text) is 'Anon-callable. THE marketplace search. Returns the FadeUp offer — Independent and Barbershop establishments — by default, at NULL, and for any unrecognised p_entity_type. A salaried barber is a public, followable, bookable-through-their-shop identity and is never an autonomous search result (MASTER_SPEC §2); pass p_entity_type = ''barber'' or ''all'' to ask for them by name.
